...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.WARNING("[INTERACTIVE] Потребуется подтверждение удаления"))
        else:
            self.stdout.write(self.style.SUCCESS("[FORCE] Автоматическое удаление включено"))
        ...
        try:
            ...
            if len(orphaned_files) > max_files:
                raise CommandError(
                    f"Найдено {len(orphaned_files)} файлов, что превышает лимит безопасности {max_files}. "
            ...
            if deleted_count == len(orphaned_files):
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"Удалено {deleted_count} из {len(orphaned_files)} файлов. "
    ...
    def _validate_environment(self):
        ...
        if not os.path.exists(dags_folder):
            raise CommandError(
                f"Директория {dags_folder} не найдена. "
        ...
        if not os.access(dags_folder, os.R_OK | os.W_OK):
            raise CommandError(
                f"Недостаточно прав для доступа к {dags_folder}. "
