...
with DAG(
    ...
    def extract_data():
        ...
        try:
            ...
            temp_path = '/opt/airflow/data/temp/etl_pipeline_17102025_extracted.parquet'
    ...
    def transform_data():
        ...
        try:
            ...
            df['processing_batch_id'] = 'etl_pipeline_17102025'
            ...
            transformed_path = '/opt/airflow/data/temp/etl_pipeline_17102025_transformed.parquet'
    ...
    def load_data():
        ...
        try:
            ...
            row_count = len(df)
            logger.info("Успешно загружено %d строк в PostgreSQL таблицу processed.my_table_1" % row_count)
            
            # Дополнительно можем выполнить проверку
            with engine.connect() as conn:
                result = conn.execute(f"SELECT COUNT(*) FROM processed.my_table_1").fetchone()
                total_rows = result[0]
                logger.info("Всего строк в таблице processed.my_table_1: %d" % total_rows)
