...
with DAG(
    ...
    def extract_data():
        ...
        try:
            ...
            temp_path = '/opt/airflow/data/temp/etl_pipeline_18102025_extracted.parquet'
    ...
    def transform_data():
        ...
        try:
            ...
            df['processing_batch_id'] = 'etl_pipeline_18102025'
            ...
            transformed_path = '/opt/airflow/data/temp/etl_pipeline_18102025_transformed.parquet'
    ...
    def load_data():
        ...
        try:
            ...
            row_count = len(df)
            logger.info("Успешно загружено %d строк в PostgreSQL таблицу processed.new_schema.new_table" % row_count)
            
            # Дополнительно можем выполнить проверку
            with engine.connect() as conn:
                result = conn.execute(f"SELECT COUNT(*) FROM processed.new_schema.new_table").fetchone()
                total_rows = result[0]
                logger.info("Всего строк в таблице processed.new_schema.new_table: %d" % total_rows)
